...
void searchControl(double probability, char seedType){
            ...
            #pragma omp parallel
			{
			//#pragma omp for private(clusterID, i, j, n, m, k, t, row, column, size,dfs, perc, combination, map, upSide, downSide, leftSide, rightSide)
            //for (k = 0; k<NTHREADS; k++)				// I think we might be able to remove this if we are using 4 threads..
            {
				if (seedType == 's') seedSite(probability, map[omp_get_thread_num()]);
				else seedBond(probability, map[omp_get_thread_num()]);
                ...
                for(j =0; j<L; j++){
                        for(i =0; i<L; i++){
                                ...
                                dfs = depthFirstSearch(i,j, clusterID, map[omp_get_thread_num()]);
                                row = 'y';
                                column = 'y';
                                if(dfs>0){
				...
				i = 0;
				for (j = 0; j<L; j++){
					upSide[j] = map[omp_get_thread_num()][i][j].flag;
				}
				i = L-1;
				for (j = 0; j<L; j++){
					downSide[j] = map[omp_get_thread_num()][i][j].flag;
				}
				j = 0;
				for (i = 0; i<L; i++){
					leftSide[i] = map[omp_get_thread_num()][i][j].flag;
				}
				j = L-1;
				for (i = 0; i<L; i++){
					rightSide[i] = map[omp_get_thread_num()][i][j].flag;
				}
				for (m = 0; m<L; m++){
					upSideP[m] = 0;
					downSideP[m] = 0;
