...
void side_update(int *numClusters, int length, int *side, struct node_cluster **refMap, struct node_cluster **superclusters,struct node_cluster **all_clusters,struct node_cluster **second_cluster, int type){
	...
	printf("::%i\n",side[Z]);
		...
		HASH_FIND_INT(*refMap, &place, link);
		printf("ding");
		if(link!=NULL){
		printf("\nIF0\n");
			...
			if((type==0)&&(place!=0)){
				printf("ELSE");
				struct node_cluster input;
				input.size=link->size;
				input.row_perc=0;
				input.col_perc=link->col_perc;
				HASH_FIND_INT(*second_cluster, &place, link);
				struct node_cluster *check;
				HASH_FIND_INT(*all_clusters, &place, check);
				if(check){
				printf("\nIF2\n");
					input.id= *numClusters;
					input.parent=*numClusters;
					HASH_ADD_INT(*refMap,id, link);
					HASH_ADD_INT(*superclusters, id, link);
					numClusters[0]++;
					HASH_ADD_INT(*all_clusters, id, &input);
				}
				else{
				printf("\nELSE2\n");
					input.id=side[Z];
					input.parent=place;
					if(side[Z]==numClusters[0]){
						numClusters[0]++;
					}
					HASH_ADD_INT(*all_clusters, id, &input);
					HASH_ADD_INT(*refMap, id, link);
...
int combi(struct node_cluster **all_clusters,struct node_cluster **second_cluster, int *highest_id, struct node_cluster *SA, int **AA, struct node_cluster *SB, int **BB, int comp_length, int A_height, int B_height){ 
	...
	printf("1\n");
	side_update(&numSupers,L*A_height, AA[0], &link_A, &superclusters,all_clusters, second_cluster,1);
	printf("1\n");
	side_update(&numSupers,L*A_height, AA[2], &link_A, &superclusters,all_clusters, second_cluster,1);
	printf("1\n");
	...
	for(int Z =0; Z<L;Z++){ // you can see that this should be iterated across all sides.
		...
		printf("\nIF0\n");
			...
			if(place!=0){
				...
				printf("\nIF2\n");
					input.id= numSupers;
					input.parent=numSupers;
					HASH_ADD_INT(link_B,id, link);
					HASH_ADD_INT(superclusters, id, link);
					numSupers++;
					HASH_ADD_INT(*all_clusters, id, &input);
				}
	...
	side_update(&numSupers,L*B_height, BB[0], &link_B, &superclusters,all_clusters, second_cluster,0);
	printf("1\n");
	side_update(&numSupers,L*B_height, BB[2], &link_B, &superclusters,all_clusters, second_cluster,0);
	printf("1\n");
	side_update(&numSupers,L*comp_length, AA[1], &link_A, &superclusters,all_clusters, second_cluster,1);
	printf("1\n");
	side_update(&numSupers,L*comp_length, BB[1], &link_B, &superclusters,all_clusters, second_cluster,0);
	printf("1\n");
	...
	for(int Z=0; Z<L*comp_length;Z++){//freeing up unused memory
		...
		if(mid){
			...
			HASH_FIND_INT(superclusters, &mid->id, mid);//memory leak here
			if((mid)&&(mid->id!=mid->parent)){
...
//this bit can be multithreaded easily. just leaving it for now though.
	...
	//this is really bad form, but 7 hrs until deadline
	int **square1 = calloc(3, sizeof(int*));
	int **square2 = calloc(3, sizeof(int*));
	struct node_cluster *side1 = calloc(2*L, sizeof(struct node_cluster));
	struct node_cluster *side2 = calloc(2*L, sizeof(struct node_cluster));
	int out[2];
	...
	//		int combi(struct node_cluster **all_clusters,int *highest_id, struct node_cluster *SA, int **AA, struct node_cluster *SB, int **BB, int comp_length, int A_height, int B_height){
		printf("HERE\n"); 
		out[MF] = combi(&all_clusters[MF],&all_clusters[MF+1],&high_ID[MF], SA, AA,SB,BB, comp_length,A_height,B_height);
		printf("HERE1\n");
		printf("%i\n",MF);
		if(MF==0){
			printf("HERE\n");
			square1[1]=calloc(2*L, sizeof(int));
			square1[0]=calloc(L, sizeof(int));
			square1[2]=calloc(L, sizeof(int));
			printf("HERE\n");
			for(int Z=0; Z<L;Z++){
				printf("%i\n",AA[0][Z]);
				square1[1][Z]=AA[0][Z];
				square1[1][Z+L]=BB[0][Z];
				square1[0][Z]=AA[1][Z];
				square1[2][Z]=BB[1][Z];
			}
			for(int Z=0;Z<L;Z++){
				struct node_cluster *out;
				HASH_FIND_INT(all_clusters[MF], &AA[2][Z], out);
				if(out){
					side1[Z].id=out->id;
					side1[Z].size=out->size;
					side1[Z].parent=out->parent;
					side1[Z].col_perc=out->col_perc;
					side1[Z].row_perc=out->row_perc;
				}else{
					side1[Z].id=0;
					side1[Z].parent = 0;
					side1[Z].size=0;
				...
				HASH_FIND_INT(all_clusters[MF], &BB[2][Z], out);
				if(out){
					side1[Z+L].id=out->id;
					side1[Z+L].size=out->size;
					side1[Z+L].parent=out->parent;
					side1[Z+L].col_perc=out->col_perc;
					side1[Z+L].row_perc=out->row_perc;
				}
				else{
					side1[Z+L].id=0;
					side1[L+Z].parent = 0;
					side1[Z+L].size=0;
					side1[Z+L].col_perc=0;
					side1[Z+L].row_perc=0;
				}
			}
		}if(MF==1){// yes could helper function this. time limit.->Ctrl+C/V less debugging.
			printf("MF 1\n");
			square2[1]=calloc(2*L, sizeof(int));
			square2[0]=calloc(L, sizeof(int));
			square2[2]=calloc(L, sizeof(int));
			printf("%i MF 1-0 %i\n", MF, AA[0][1]);
			for(int Z=0; Z<L;Z++){
				printf("CHEKC%i\n",AA[0][Z]);
				square2[1][Z]=AA[0][Z];
				square2[1][Z+L]=BB[0][Z];
				square2[0][Z]=AA[1][Z];
				square2[2][Z]=BB[1][Z];
			}
			/*
			memcpy(square2[1], AA[0],(L*sizeof(int)));
			memcpy(&(square2[1])[L], BB[0], (L*sizeof(int)));
			memcpy(square2[0], AA[1],(L*sizeof(int)));
			memcpy(square2[2], BB[1],(L*sizeof(int)));*/
			printf("MF 1-2\n");
			for(int Z=0;Z<L;Z++){
				printf("MF 1-2\n");
				struct node_cluster *out;
				...
				if(out){
					printf("BANANA%i\n",out->id);
					side2[Z].id=out->id;
					side2[Z].size=out->size;
					side2[Z].parent=out->parent;
					side2[Z].col_perc=out->col_perc;
					side2[Z].row_perc=out->row_perc;
				}else{
					printf("NOT THERE\n");
					side2[Z].id=0;
					side2[Z].parent = 0;
				...
				if(out){
					side2[Z+L].id=out->id;
					side2[Z+L].size=out->size;
					side2[Z+L].parent=out->parent;
					side2[Z+L].col_perc=out->col_perc;
					side2[Z+L].row_perc=out->row_perc;
				}
				else{
					side2[Z+L].id=0;
					side2[L+Z].parent = 0;
					side2[Z+L].size=0;
					side2[Z+L].col_perc=0;
					side2[Z+L].row_perc=0;
				}
			}
		}
		//free(SA);
	...
	printf("HELLO");
	out[1]=combi(&all_clusters[0],&all_clusters[2],&out[0], side1, square1,side2,square2, 2,1,1);
	printf("HELLO");
