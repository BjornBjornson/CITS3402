...
int combi(struct node_cluster **all_clusters,int *highest_id, /*struct node_cluster ?*/int *SA, int **AA, /*struct node_cluster*/int *SB, int **BB, int comp_length, int A_height, int B_height){ //tried to wrangle it- failed. should be simple - passing two variable sized arrays of int[L] arrays, and one integer.
...
//multithread this bit
	...
	for(int i=0; i<NTHREADS;i++){
		for(int j=2;j<high_ID[i];j++){
			...
			initialising=malloc(sizeof(struct node_cluster));
			initialising->id=i;
